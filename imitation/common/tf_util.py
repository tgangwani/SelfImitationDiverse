...
def get_available_gpus(session_config=None):
    ...
    if session_config is None:
        session_config = get_session()._config   # pylint: disable=protected-access
...
class logRecord():
    def __init__(self, path):
        ...
        self.path = path
        if path is None:
            print('Data dump path not provided. Plotting data would not be saved !!')
        else:
    ...
    def dump(self):
        if self.path is not None:
            ...
            print('Data dump saving complete. Path: %s.npy' % self.path)
